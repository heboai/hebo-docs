...
The test case parser supports system messages that can be used to set the context or behavior of the AI agent. System messages:

- Ability to start with `System:` followed by the message content
- Can only appear at the start of a conversation
- Are handled differently by different agent types:
  - **OpenAI Agents**: System messages are included in the message history and sent as the first message in the API request
  - **Hebo Agents**: System messages are ignored with a warning message ("System messages are not supported by Hebo agents and will be ignored.")
...
7. When using system messages:
   - Place them at the start of each test case
   - Keep them concise and focused on the specific test scenario
   - Be aware that they will be ignored by Hebo agents
   - Use them to set clear expectations for OpenAI agent behavior
